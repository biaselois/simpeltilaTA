...
@if (Auth::user()->role == 'admin')
        <form action="{{ route('submissions.distribute') }}" method="POST" id="distributeForm">
            @csrf
            <button type="submit" class="btn btn-success">
                <i class="fas fa-share-alt"></i> Bagikan Tugas
            </button>
        </form>
    @endif
    <div class="card">
                ...
                <tbody>
                    @foreach ($datas as $data)
                        <tr>
                            ...
                            <td>{{ $data->email}}</td>
                            <td>
                                <a href="https://wa.me/{{ ltrim($data->nohp, '0') }}" target="_blank">{{ $data->nohp }}</a>
                            </td>
                            <td>{{ $data->namappat}}</td>
                            ...
                            <td>{{ $data->listrik}}</td>
                            <td>{{ $data->tahundibangun}}</td>
                            <td> <a href="{{ asset('storage/' . $data->fotoobjek) }}" target="_blank">Lihat fotoobjek</a></td>
                            <td> <a href="{{ asset('storage/' . $data->sertifikat) }}" target="_blank">Lihat Sertifikat</a></td>
                            <td> <a href="{{ asset('storage/' . $data->sppt) }}" target="_blank">Lihat SPPT</a></td>
                            <td>{{ $data->bidang }}</td>
            ...
            $('.btn-delete').click(function() {
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        ...
                        $.ajax({
                            type: 'DELETE',
                            url: "{{ route('submission-list.delete', ':id') }}".replace(':id', id), // URL dinamis dengan ID
                            dataType: 'JSON',
                            data: {
                                'id': id,
                                '_token': '{{ csrf_token() }}',
